...
import { createTag, deleteTag, getAdminTags, updateTag } from "@/lib/db/tags";
import type { AdminTag, AdminTagInput } from "@/types/tag";

type PageStatus = "loading" | "ready";

const initialForm: AdminTagInput = {
  name: "",
  slug: "",
};
...
export default function AdminTagsPage() {
  ...
  const [formValues, setFormValues] = useState<AdminTagInput>(initialForm);
  ...
  useEffect(() => {
    ...
    async function loadTags() {
      ...
      if (!result.success) {
        showMessage(result.error || "标签列表读取失败，请稍后重试。", "error");
      }
  ...
  async function reloadTags() {
    ...
    if (!result.success) {
      showMessage(result.error || "标签列表刷新失败，请稍后重试。", "error");
      return;
    }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const validationMessage = validateTag(formValues);
    ...
    resetForm();
    await reloadTags();
    showMessage(wasEditing ? "标签已更新。" : "标签已新增。", "info");
    setIsSubmitting(false);
  }

  async function handleDelete(tag: AdminTag) {
    const tagName = tag.name || "未命名标签";
    const confirmed = window.confirm(`确认删除「${tagName}」吗？如果该标签正在被工具或文章使用，删除会失败。`);
    ...
    if (!result.success) {
      showMessage(result.error || "该标签可能正在被工具或文章使用，请先调整相关内容后再删除。", "error");
      setDeletingId("");
      return;
    }
  ...
  return (
    <AdminShell>
      <section className="glass-card-strong p-5 sm:p-6">
        <div>
          <p className="text-sm font-bold text-sky-700">标签内容</p>
          <h1 className="mt-2 text-2xl font-black text-ink sm:text-3xl">标签管理</h1>
          <p className="mt-2 text-sm leading-7 text-slate-600">管理工具和文章使用的内容标签。</p>
        </div>
      </section>
      ...
      <section className="glass-card-strong p-5 sm:p-6">
        <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-sm font-bold text-sky-700">{isEditing ? "编辑标签" : "新增标签区域"}</p>
            <h2 className="mt-2 text-xl font-black text-ink">{isEditing ? "修改当前标签" : "新增一个内容标签"}</h2>
            <p className="mt-2 text-sm leading-7 text-slate-600">
              标签会用于前台筛选、搜索匹配和后续内容推荐。本阶段只维护标签本身，不做工具或文章标签绑定。
            </p>
          </div>
          {isEditing ? (
            <button
        ...
        <form className="grid gap-4" method="post" onSubmit={handleSubmit} noValidate>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">
            ...
            >
              {isSubmitting ? "保存中..." : isEditing ? "保存修改" : "新增标签"}
            </button>
          </div>
        </form>
      </section>
      ...
      <section className="glass-card p-5 sm:p-6">
        <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2 className="text-xl font-black text-ink">标签列表</h2>
            <p className="mt-2 text-sm leading-7 text-slate-600">
              {status === "loading" ? "正在读取标签..." : `当前共 ${sortedTags.length} 个标签`}
            </p>
          </div>
        </div>

        {status === "ready" && sortedTags.length === 0 ? (
          <div className="rounded-3xl border border-white/75 bg-white/65 p-8 text-center">
            <h3 className="text-xl font-black text-ink">暂无标签</h3>
            <p className="mt-3 text-sm leading-7 text-slate-600">
              可以先新增一个标签，后续再将它绑定到工具或文章。
            </p>
          </div>
        ) : (
          <div className="grid gap-3">
            {sortedTags.map((tag) => (
              <article key={tag.id} className="rounded-3xl border border-white/75 bg-white/70 p-4 shadow-sm sm:p-5">
                <div className="grid gap-4 lg:grid-cols-[1fr_auto] lg:items-center">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <h3 className="text-lg font-black text-ink">{tag.name || "未命名标签"}</h3>
                      <span className="rounded-full bg-sky-50 px-3 py-1 text-xs font-bold text-sky-700">
                        {tag.slug || "-"}
                      </span>
                    </div>
                    <p className="mt-2 text-xs font-semibold text-slate-400">创建时间：{formatDate(tag.created_at)}</p>
                  </div>
...
function validateTag(values: AdminTagInput) {
  if (!values.name.trim()) {
    return "标签名称不能为空。";
  }
