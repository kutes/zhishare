// Creates the public "article-covers" storage bucket (generated SVG covers for articles).
...
if (!executeMode) {
  console.log("CREATE_ARTICLE_BUCKET_DRY_RUN_OK");
...
if (existing) {
  console.log("ARTICLE_BUCKET_ALREADY_EXISTS");
  process.exit(0);
}

const { error } = await client.storage.createBucket(BUCKET, { public: true });
if (error) {
  console.error(`CREATE_ARTICLE_BUCKET_FAILED: ${error.message}`);
  process.exit(1);
}
console.log("CREATE_ARTICLE_BUCKET_OK");
