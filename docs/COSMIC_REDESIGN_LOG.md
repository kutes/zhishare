...
### Step 61 - Tool detail page polish (mirrors article page treatment)

Status: completed

- Hero slimmed: title clamp(2.7-4.8rem)→clamp(1.9-2.75rem); duplicate description paragraph and the self-referential note removed from the hero card (decision panel stays — it is real value); QuickFactsBar deleted entirely (收录状态:已发布 / 建议动作:先看简介 were dev jargon; category already shows as a badge).
- Main content became a centered 860px white "paper" card reusing the --ad-* variable set (scoped to .tool-detail-article), with light overrides for sections, warning section, list items, collapsible description (incl. fade gradient), tool-media blocks and inline ad. Ad-only 320px sidebar column removed.
- Real-data fix: normalizeTool no longer fabricates a features fallback ("核心功能请以官方网站或项目文档为准。") — the 核心功能 section now renders only when real data exists (currently never, since tools table has no features column).
- Mobile: junk stats row (分类 6 项/信息 N 条/模式 折叠) and 暂无标签 placeholder pill removed; collapsible sections unchanged.
- Dead CSS removed in the same pass: tool-detail-intro/note/facts/fact*/tag-empty/lead + mobile stats rules + facts in shared selectors and media queries.
- Verified: tsc green, brace balance, DOM assertions on chatgpt+cyberchef (no placeholder/jargon strings), desktop+mobile screenshots.

### Step 62 - Tool detail full redesign (unify with article layout)

Status: completed

- Problem (user screenshot): hero was a two-column split (small left card + tall right decision panel) that left a huge dead gap on the left; the decision panel's 适合人群/使用场景/风险提醒 rows duplicated the white article card's lists verbatim; overall imbalanced and cluttered.
- Redesign mirrors the article detail page: header is now a plain centered stack (badges → title → summary → tags → action buttons), no hero card. The decision panel is deleted entirely — its only unique value, the 访问官网 / 下载 buttons, moved into the centered header (rendered only when the URL exists). Body is a single centered 860px white "paper" article card used identically on desktop and mobile.
- Deleted 3 now-dead components (were desktop/mobile duplication): ToolDecisionPanel, ToolMobileSummaryCard, MobileToolDetailSections; plus the in-file MobileRelatedToolsCompact. The responsive single-column layout replaces all mobile-specific markup.
- Purged all dead CSS in the same pass: entire tool-decision-* and tool-mobile-* blocks, tool-detail-mobile-related-*, tool-detail-hero-card/intro/note/facts, section-head-mobile helpers, and their appearances in shared multi-selector rules and media queries. Net -1291 lines.
- Verified: tsc green, brace balance, DOM assertions on chatgpt+localsend (no decision-panel/mobile-sections markup, hero-plain + action button present), desktop+mobile screenshots — clean and balanced on both.
- Correction per user feedback (same day): the full-light page was too much — reverted, then re-scoped the --ad-* variable set to `.article-detail-main` only. The reading card is now a white "paper" panel while the page/hero/sidebar/related stay warm dark. Main-only classes (tldr/lead/big/why/keypoint/list/section-body etc.) reference `var(--ad-*, <dark fallback>)` so they render dark if ever used outside the card; shared classes get `.article-detail-main`-scoped overrides. Verified with fresh desktop+mobile screenshots.
- Second correction round (user: hero too big, sidebar squeezes content): hero card + cover band removed entirely — detail header is now a plain centered stack (kicker/title/summary/meta/tags) on the dark background; detail shell narrowed 1320→1080px; reading card centered at max 860px full-width (layout is single column). The 阅读概览 sidebar became a right-edge vertical tab (fixed, ≥1101px) that reveals the panel on :hover/:focus-within; below 1101px it stays static after the article (mobile has no hover). Dead CSS for hero-card/cover removed in the same pass. capture-articles.mjs now passes animations:"disabled" — CSS transitions were being caught mid-flight by fullPage screenshots and made the hidden drawer look semi-transparent (verified real computed state: opacity 0/hidden at rest, 1/visible on hover).
