...
3. **媒体检查（强制）**：官网/GitHub 有没有可用的真实截图？有就必须抓，配进 media 画廊；封面（cover_image / tool-cover-shots）必须是 Playwright 实拍官网首页或官方素材，禁止用系统兜底生成的字母占位图。确认官方确实没有可用素材后，才允许只用生成封面。
4. 跑守门脚本 `enrich-tool-detail.mjs`（dry-run 默认）：转存图片 → 上传 media JSON → 更新 tools 行 → 回读验证
5. 本地开发服务器目检详情页，同时去工具列表页肉眼确认卡片封面是真图不是占位符
6. 存量工具逐批补齐，新批次直接按本标准写

## 发布前检查清单（每次发布强制过一遍，不许跳过）

- [ ] 正文/详情页至少有 1 张真实配图（工具截图或图表），或已确认官方无可用素材
- [ ] 封面是真实抓取图，不是自动生成的字母/占位 SVG
- [ ] 所有图片/视频标注「来源：XXX」
- [ ] 发布后去列表页截图/肉眼确认卡片显示正常（不是占位符、没有 404）
- [ ] 无法满足以上任一项时，在交付说明里明确写清楚原因，不能默不作声地跳过

**2026-07-18 教训**：kimi-k3-hidden-prompt 一文发布时漏了媒体检查，正文零配图、封面用了占位符，而原始信源其实有真实素材可抓（K3 生成的鹈鹕图）。后补修复。此清单自此为强制项，适用于文章和工具两条线。
