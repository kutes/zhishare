...
- `docs/content/article-drafts/free-ai-tools-safety.md`
- `docs/content/article-drafts/netdisk-resource-risks.md`
- `docs/content/article-drafts/free-image-tools-comparison.md`
...
审核 3 篇文章草稿。确认无问题后，再按现有 articles 表字段录入后台或 Supabase。

## 2026-06-12

### 本次任务

将第二阶段 3 篇文章样板草稿录入 Supabase `articles` 表，并发布到前台。
...
- 3 篇新文章状态设置为 `published`。
- 旧的 2 篇 `published` 文章改回 `draft`，未删除。
- 文章内容来自 `docs/content/article-drafts/` 草稿文件。

### 验收结果

- `/articles` 只显示当前 3 篇新文章。
- 3 个详情页均可访问。
- 页面标题、摘要和正文内容已显示。
- 页面未发现乱码、`undefined` 或 `null`。
- build 通过。
...
完成第四阶段 `/tools` 工具列表页第一轮优化最终验收。

### 验收结果

- `/tools` 显示 10 个 published 工具。
- 新 5 个工具与旧 5 个工具均正常展示。
- 移动端 Hero 更紧凑。
- 搜索与筛选区更轻。
- 分类 chips 移动端浏览正常。
- 工具卡片信息结构更适合小白快速判断。
...
- 工具数量达到 10 个后，优化 `/tools` 的发现体验。
- 收紧 Hero 和筛选区高度。
- 强化“按任务找工具”的入口。
- 优化移动端工具卡片密度。
- 优化搜索、分类 chips 和空状态。
- 不新增用户系统、互动功能、排行榜或数据库字段。
