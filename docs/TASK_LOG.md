...
## 2026-07-14 工具详情副属性隐藏式 + 后台管理页审核翻新

Result: completed(#1 隐藏式、#2 后台翻新+文案);#3 官方下载后台字段因加列门槛不做

- 副属性隐藏式:适合人群/使用场景/优缺点收进「更多信息」折叠块(CollapsibleBlock 客户端组件,默认收起),白卡只留"这个工具是什么"+折叠条+风险提醒+按钮;页面更短更聚焦。
- 后台管理页审核 + 翻新:
  - 视觉:后台仍停在改版前的蓝色主题(sky-blue/浅蓝底/glass-card 深卡)。改法=给后台根节点加 .admin-theme 作用域,在 globals.css 里只覆盖后台内的蓝→暖(glass-card 转暖白、section-gradient-soft/blue 透明、sky-* 文字/底/边→amber、.admin-theme 根设暖米底),一处改动带动全后台、不碰前台(旧类仍被前台搜索页/页脚使用)。仪表盘 4 张统计卡渐变改琥珀。登录页截图确认暖色生效。
  - 文案(#2):封面字段占位「当前阶段仅预留字段」过时→改"留空自动生成/可粘贴直链";英文报错→中文。
  - 排查:一次黑底是 CSS 热重载未完成的旧帧;真因是登录页用 section-gradient-blue(非 soft),补覆盖后修复。
- #3 官方下载后台字段:需给 tools 表加列,只能 Supabase 控制台跑 SQL(用户不便做);且官方下载已可由 set-tool-official-download 脚本设置,后台字段非必需——不做,避免上线一个填了不生效的坏字段。
- 验证:tsc 全绿、括号平衡、file-browser 详情折叠态截图、admin 登录页暖色截图。仍未推送线上。

## 2026-07-14 文章卡去图重设计 + 工具/文章整卡可点

Result: completed

- 文章卡重设计:去掉封面图(避免"我试/我为"这类尴尬字母封面),改纯排版创意卡——顶部分类+日期、大号衬线标题、摘要、底部阅读时长+阅读全文,配大号淡雅衬线引号水印 + 顶部琥珀强调条(hover 横向展开、箭头右移、标题提亮)。
- 整卡可点(工具+文章):根因是 .articles-card > * / .zh-tool-card 把"阅读全文/箭头"链接设成 position:relative,导致 stretched-link 的 ::after{inset:0} 只覆盖链接本身、而非整卡——用户只能点那一小块。修法=把整张卡直接做成 <Link>,内部箭头/阅读链接降级为装饰 span,最稳、无伪元素依赖。
- 死 CSS 清理:.zh-tool-link 及其 ::after 全站已无引用(tsx 零匹配)→ 从共享分组摘除;文章卡 banner/cover-fallback/meta/link 规则删除。zh-tool-link-secondary/zh-tools-button 仍在用,保留。给 .zh-tool-card/.articles-card 加 text-decoration:none。
- 验证:tsc 全绿、括号平衡、无真死类;Playwright 点击卡片主体确认跳转(文章→/articles/xxx、工具→/tools/xxx);首页截图确认新卡样式。仍未推送线上。
